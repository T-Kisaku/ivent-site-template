/**
 * This source code is imported from https://www.npmjs.com/package/three.interactive
 * Because when this app is built, this package occor error... ...
 */
...
export class InteractionManager {
	...
	remove = (object: THREE.Object3D, childNames: string[] = []) => {
		if (!object) return;
		const filterSet = new Set<string>(childNames.length > 0 ? childNames : [object.name]);

		this.interactiveObjects = this.interactiveObjects.filter((o) => !filterSet.has(o.name));
	};
